...
public enum AudioSceneProjector {
    ...
    ///
    /// Authored boss phases follow the core's authoritative phase identity. The
    /// health-band fallback supports older/synthetic states that predate phase
    /// exposure without allowing presentation to alter simulation behavior.
    ...
    /// Maps any authored phase count onto the available music progression. This
    /// permits Atlanta's six simulation phases to advance through four produced
    /// music movements while preserving monotonic phase order.
    static func phaseIndex(for phase: BossPhase, assetCount: Int) -> Int {
        guard assetCount > 1, phase.count > 0 else { return 0 }
        let index = phase.ordinal * assetCount / phase.count
