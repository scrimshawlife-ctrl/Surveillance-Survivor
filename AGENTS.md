...
- **`docs/AUDIO_PLAN.md` — START HERE for all audio work** (status, batch order, 17 stems, links).
...
3. The 17 stems in `Sources/SurveillanceCore/Resources/Content/audio_events.json` are the only currently runtime-addressable product cues unless code, catalog entries, and tests are extended together.
