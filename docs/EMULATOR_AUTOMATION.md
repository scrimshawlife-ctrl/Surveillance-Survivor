...
| `make launch-smoke` | **No `-UITesting`**: splash → start menu → BEGIN RUN → play chrome (LaunchShellUITests) |
...
8. **XCUITests** — 14 black-box journeys: launch chrome (under `-UITesting`), pause/resume, settings, VoiceOver semantics, reduced-motion persistence, upgrade selection, extraction, defeat, daily/weekly challenge launch, dense-combat rendering, mechanical force-extract receipt presentation, and **launch-shell smoke without `-UITesting`** (splash → start menu → BEGIN RUN).
...
9b. **Launch-shell smoke** — `make launch-smoke` runs `LaunchShellUITests` only; production launch path (no `-UITesting`). Artifacts under `.launch-smoke/`.
...
15. **Unified QA index** — `qa/non-device-baseline.json` is the point-in-time authority for the 268 package / 397 simulator / 14 UI baseline. Every matrix run validates it and emits `qa-index.json`, `.md`, and `.html`, linking the contact sheet, receipts, trend report, and anomaly review from one page. Commit mismatches or missing evidence fail closed.
