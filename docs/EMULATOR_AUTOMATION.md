...
15. **Unified QA index** — `qa/non-device-baseline.json` is the point-in-time authority for the 229 package / 347 simulator / 10 UI baseline. Every matrix run validates it and emits `qa-index.json`, `.md`, and `.html`, linking the contact sheet, receipts, trend report, and anomaly review from one page. Commit mismatches or missing evidence fail closed.
