...
| --- | --- | --- | --- | --- | --- | --- |
| **G-01** | G | **S1** | Sidewalks / curb paint may vanish under nearly opaque terrain carpet; layer stack “fucked”; possible z-fight / visual chaos | Was: `stampTerrainLayer` `.full` used `root.addChild`. **Fixed:** `parent.addChild` + name `urban-terrain-tile`; regression `worldProjectorFullTerrainCarpetParentsUnderUrbanGround` **PASS** (448 sim tests). | WorldProjector terrain carpet | **FIXED** on main (2026-08-05) |
| **G-02** | G | **S1** | Floor busy / noisy / “wallpaper” at combat scale | Was α **0.88**. **Mitigated:** primary carpet α **0.48**, secondary edge **0.18** (2026-08-05). Re-check on device. | WorldProjector | **MITIGATED** — operator eyes still owed |
| **G-03** | G | **S1** | Player “pops” / changes outfit while walking | Canvas was unified; content feet/height still drifted by facing. **Feet/scale normalize** on 24 walk+idle PNGs (receipt). Outfit wardrobe identity still needs art regen. | Player walk banks | **PARTIAL** — feet locked; wardrobe art residual |
