...
**Agents and the board** treat the machine file as source of truth for “are we ship-blocked, and on what?” **Humans** fill evidence slots—device logs, ART checklist rows, live store URLs, licensed audio stems, GitHub #3 ship notes. Agents prepare structure, enforce tip match, demote when the tip moves, and promote to `READY` only when the honesty checker would pass.
...
| `audio_product` | [`docs/AUDIO_ASSET_MANIFEST.json`](../AUDIO_ASSET_MANIFEST.json) · [`docs/AUDIO_PLAN.md`](../AUDIO_PLAN.md) · [`docs/AUDIO_ASSET_PRODUCTION_BIBLE.md`](../AUDIO_ASSET_PRODUCTION_BIBLE.md) · related audio docs under `docs/` | Licensed product stems (e.g. ElevenLabs license → Audio Batch 1) present as product assets. Catalog-only is **not** READY. **Never** system-sound placeholders |
