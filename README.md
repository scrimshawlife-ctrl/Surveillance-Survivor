...
> **Development status:** active **pre-alpha**. Simulator-ready vertical slice with deterministic core, ten-city district profiles, authored interactable cascades, player-facing boss phases, campaign unlocks, visual asset map, global environment package v1, and **all ten city foundation packs** on `main` (**194 validated runtime PNGs**). The complete 68-asset mastered and delivered audio bank is runtime-integrated; physical-device listening and routing acceptance remain open. Current non-device evidence passes **252 package tests, 381 simulator-hosted tests, 13 UI journeys, and a 20/20 combat/reduced visual matrix**. **Not release-ready** because physical-device acceptance and App Store owner fields remain open. Live board: [`docs/REPO_STATUS.md`](docs/REPO_STATUS.md).
...
| Emulator automation | **Emulator-verified: 252 package + 381 simulator + 13 UI tests; 20/20 visual panels** |
